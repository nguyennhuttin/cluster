...
save_path_signals = os.path.join(folder_dir+"/data", data_name + "_signals.pt")
save_path_labels = os.path.join(folder_dir+"/data", data_name + "_labels.pt")
save_path_real_signals = os.path.join(
    folder_dir+"/data", data_name + "real_signals.pt")
save_path_real_labels = os.path.join(
    folder_dir+"/data", data_name + "real_labels.pt")
printLog('s', 'l', 'rs', 'rl')
printLog(save_path_signals, save_path_labels,
         save_path_real_signals, save_path_real_labels)
...
if gen_data_real:
    matlabfile = loadmat(os.path.join(
        folder_dir, 'SRP-Test-Data/full_data_pow2_224_clstm.mat'))
...
if cpu_mode == False:
    ...
    data_loader_train = DataLoader(
        dataset=dataset_train, sampler=train_sampler, batch_size=batch_size, shuffle=True)
    ...
    printLog("Input data shape:", data.shape)
    printLog("Labels shape:", labels.shape)
