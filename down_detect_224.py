...
#! DOWN SAMPLE #####################################################
#! #################################################################
y_data = signal.resample(y_data, 200, t=None, axis=1, window=None)
...
#! DOWN SAMPLE #####################################################
#! #################################################################
test_labels = signal.resample(test_labels, 200, t=None, axis=1, window=None)
...
if cpu_mode == False:
    ...
    class CNN_LSTM(nn.Module):
        def __init__(self, dim_out, layer_type1=DilatedTempConv, layer_type2=TempConv, num_blocks1=1, num_blocks2=1):
            ...
            self.layers11 = self.create_blocks(
                num_blocks=num_blocks1, block_type=layer_type1, channels_in=1, channels_out=16)
            ...
            self.bilstm = nn.LSTM(input_size=36, hidden_size=200 //
    ...
    def evaluate(net, device, loader):
        ...
        return epoch_acc / (len(loader.dataset)*200)
...
# %%
    ...
    def evaluate2(net, device, loader):
        ...
        # return the accuracy from the epoch
        acc = epoch_acc / (len(loader.dataset)*200)
        return [fx, y, acc]
